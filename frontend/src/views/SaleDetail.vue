      ...
      <div style="display:flex;flex-direction:column;gap:12px;">
        <div style="display:flex;justify-content:space-between;align-items:center;gap:12px;">
          ...
          <div style="display:flex;gap:8px;align-items:center;">
            ...
            <el-button type="primary" @click="continueCreate">继续开单</el-button>
            <el-button link type="primary" @click="continueWithCurrentCustomer">沿用当前客户</el-button>
            <el-button v-if="mainAction" @click="mainAction.handler">{{ mainAction.label }}</el-button>
        ...
        >
          <template #default>
            <div style="display:flex;gap:8px;margin-top:8px;">
              <el-button size="small" type="primary" @click="continueCreate">继续开单</el-button>
              <el-button size="small" link type="primary" @click="continueWithCurrentCustomer">沿用当前客户</el-button>
            </div>
          </template>
        </el-alert>
      </div>
...
import { exportSaleExcel, generateDelivery, getSaleApi, getSalePaymentRecords } from '../api/sales'
...
const mainAction = computed(() => {
  ...
  if (sale.value.order_stage === 'QUOTE') return { label: '转为销售单', handler: convertQuoteToSale }
  if (sale.value.order_stage === 'SALE_CONFIRMED' && sale.value.ar_amount > 0) {
    return { label: sale.value.paid_amount > 0 ? '继续收款' : '去收款', handler: () => router.push(`/sales/${sale.value.id}/payment`) }
  }
  if (sale.value.order_stage === 'SALE_CONFIRMED' && sale.value.needs_delivery && sale.value.delivery_status !== 'GENERATED') {
    return { label: '生成送货单', handler: openDeliveryDialog }
  }
  if (sale.value.order_stage === 'DELIVERY_CREATED') {
    return { label: '导出送货单', handler: () => downloadDoc('delivery') }
  }
  return null
})
...
function continueWithCurrentCustomer() {
  wizardStore.clearDraft()
  wizardStore.orderType = 'retail'
  ...
  wizardStore.sourceQuoteId = null
...
function convertQuoteToSale() {
  ...
  wizardStore.items = (sale.value.items || []).map((item) => ({
  ...
  wizardStore.sourceQuoteId = sale.value.id
  wizardStore.settlement = {
    settlement_status: 'PAID',
    paid_amount: sale.value.total_amount || 0,
    payment_method: 'bank_transfer',
    payment_note: ''
  }
