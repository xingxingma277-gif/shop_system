...
from app.services import auth_service, customer_contact_service

router = APIRouter(prefix="/api", tags=["Contacts"])


@router.get("/customers/{customer_id}/contacts", response_model=Page[CustomerContactRead])
def list_contacts(customer_id: int, page: int = 1, page_size: int = 20,
...
def create_contact(customer_id: int, payload: CustomerContactCreate,
                   _: dict | None = Depends(auth_service.require_permissions('customer.manage')),
                   session: Session = Depends(get_session)):
    try:
...
def update_contact(contact_id: int, payload: CustomerContactUpdate,
                   _: dict | None = Depends(auth_service.require_permissions('customer.manage')),
                   session: Session = Depends(get_session)):
    try:
...
def toggle_contact(contact_id: int,
                   _: dict | None = Depends(auth_service.require_permissions('customer.manage')),
                   session: Session = Depends(get_session)):
    try:
