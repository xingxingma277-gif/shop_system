...
from app.services import auth_service, supplier_service
...
def create_supplier(payload: SupplierCreate, _: dict | None = Depends(auth_service.require_permissions('supplier.manage')), session: Session = Depends(get_session)):
    try:
...
def update_supplier(supplier_id: int, payload: SupplierUpdate, _: dict | None = Depends(auth_service.require_permissions('supplier.manage')), session: Session = Depends(get_session)):
    try:
