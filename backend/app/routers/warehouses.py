...
def list_warehouses(status: str | None = Query(None), session: Session = Depends(get_session)):
    return warehouse_service.list_warehouses(session, status=status)


@router.post('', response_model=WarehouseRead)
def create_warehouse(payload: WarehouseCreate, session: Session = Depends(get_session)):
    try:
...
def update_warehouse(warehouse_id: int, payload: WarehouseUpdate, session: Session = Depends(get_session)):
    try:
